...
path = "C:\\Users\\Simon\\Downloads\\augm\\Original_high"
path2 = "C:\\Users\\Simon\\Downloads\\augm\\Original_high_augm"
...
def crop_image(path):
    ...
    for file in files:
        os.chdir(path)
        img = imread(file) # uint8
        img_augm = img[:120, :160, :]  # high: 240x320, low: 120x160
        img_augm = cv2.resize(img_augm, (480, 320), interpolation=cv2.INTER_NEAREST) # high: 640x480, low: 480x320
        end = file.find(".")
...
def dropout_image(path):
    os.chdir(path)
    files = os.listdir() # np.random.seed(1000)
    for file in files:
        os.chdir(path)
        img = imread(file) # uint8
        img_augm = img
        begin = 50 # np.random.randint(20, 300)
        dist = 25
        img_augm[begin:begin+dist,begin:begin+dist,:] = 0
        begin2 = 190 #np.random.randint(250, 450)
        dist = 35
